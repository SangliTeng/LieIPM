...
% lam: multipliers for the dynamics, i.e., the constrained force, in this case, merge this with control input u.
% manifold M = M_{RB} x M_{u} + M_{\lam}

elam = SX.sym('elam', [dof, 1]);
% nu   = SX.sym('nu',  [length(emat(:)) / 2 + length(lam), 1]);
% nu: multipliers for the optimizations
nu   = SX.sym('nu',  [length(emat(:)) / 2 + dof, 1]); 
