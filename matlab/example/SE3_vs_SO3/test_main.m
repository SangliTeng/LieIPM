...
parfor k = 1:num_trial
    ...
    QR = diag([1, 1, 1]) * 1 * scale;
    QF = diag([1, 1, 1]) * 20 * scale;
    Qp = diag([1, 1, 1]) * 1 * scale;
    Qv = diag([1, 1, 1]) * 1 * scale;
    ...
    % W = diag([1, 1, 1, 1]) * 10;
    W = diag([50, 50, 50, 1, 1, 1]) * .05 * scale;
    ...
    %%
    X_op = StaticInit(X0, Xg_init, Ns);
    U_op = {};
    for kk = 1:Ns+1
        if kk <= Ns
            U_op{kk}   = [randn(5, 1) * 0.0; 9.81 * .1] * 0 + 0.68 * 0;
        end
    end
    y     = randn(Ns * utils.Nh + utils.Nx0, 1) * 0.0; % dual variable
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % save initialization only
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % cpp_data = struct();
    % cpp_data.x = [vars.x(:);vars.u(:)];
    % cpp_data.y = y;
    % cpp_data.z = z;
    % cpp_data.s = s;
    % cpp_data.mu = sigma;
    ...
    % 
    % save(sprintf('cpp_test_in/init_%d.mat', k), '-struct', 'cpp_data');
...
plot(1:100, (1:100) * 0 + 1e-6, 'k--', LineWidth=1)
% xlim([0, 20])
set(gca, 'YScale', 'log')
